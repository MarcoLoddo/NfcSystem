...
import it.extrasys.tesi.tagsystem.corner_service.test.JsonConverter;
...
public class CornerServiceUnityTest {
  ...
  public void getAllCornersTest() throws Exception {
    ...
    this.mockMvc.perform(get("/corners/")).andExpect(status().isOk()).andExpect(content().contentType(this.contentType))
        .andExpect(jsonPath("$", Matchers.hasSize(1))).andExpect(jsonPath("$[0].cornerId", is(0)));
    ...
    Mockito.when(this.cornerManager.getAll()).thenReturn(list);
    this.mockMvc.perform(get("/corners/")).andExpect(status().isOk()).andExpect(content().contentType(this.contentType))
        .andExpect(jsonPath("$", Matchers.hasSize(1))).andExpect(jsonPath("$[0].cornerId", is(1)))
  ...
  public void getCornerByIdTest() throws Exception {
    ...
    this.mockMvc.perform(get("/corners/0")).andExpect(status().isOk())
        .andExpect(content().contentType(this.contentType)).andExpect(jsonPath("$.cornerId", is(0)))
  ...
  public void addCornerTest() throws IOException, Exception {
    ...
    Mockito.when(this.cornerManager.add(any(CornerEntity.class))).thenReturn(mockId(test));
    this.mockMvc
        .perform(post("/corners/").contentType(this.contentType)
            .content(this.jsonConverter.json(CornerDtoConverter.toDto(createCornerEntity()))))
  ...
  public void updateCornerTest() throws IOException, Exception {
    ...
    Mockito.when(this.cornerManager.updateReader(anyLong(), anyString())).thenReturn(null);
    Mockito.when(this.cornerManager.updateMealId(anyLong(), anyLong())).thenReturn(null);
    Mockito.when(this.cornerManager.getById(anyLong())).thenReturn(testUpdated);
    this.mockMvc
        .perform(put("/corners/0?mealId=2&readerId=prova0").contentType(this.contentType)
            .content(this.jsonConverter.json(CornerDtoConverter.toDto(test))))
        .andExpect(status().isOk()).andExpect(jsonPath("$.cornerId", is(0))).andExpect(jsonPath("$.mealId", is(2)))
  ...
  public void addNfcReaderTest() throws IOException, Exception {
    ...
    Mockito.when(this.readerManager.add(any(NfcReaderEntity.class))).thenReturn(test);
    this.mockMvc
        .perform(post("/readers/").contentType(this.contentType)
            .content(this.jsonConverter.json(NfcReaderDtoConverter.toDto(createNfcEntity()))))
        .andExpect(status().isOk()).andExpect(content().contentType(this.contentType))
  ...
  public void updateNfcReaderTest() throws IOException, Exception {
    ...
    Mockito.when(this.readerManager.update(any(NfcReaderEntity.class))).thenReturn(test);
    this.mockMvc
