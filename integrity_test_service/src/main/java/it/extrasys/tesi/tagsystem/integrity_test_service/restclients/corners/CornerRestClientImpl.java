...
public class CornerRestClientImpl implements CornerRestClient {
  ...
  public CornerDto addCorner(CornerDto cornerDto) {
    ...
    CornerDto cornerPersisted = this.restTemplate.postForEntity(uri, cornerDto, CornerDto.class).getBody();
  ...
  public CornerDto getCornerById(Long id) {
    ...
    map.put("id", id);
    return this.restTemplate.getForEntity(uri, CornerDto.class, map).getBody();
  }
   ...
   */
  @Override
  public CornerDto updateCorner(CornerDto cornerDto, long mealId, String readerId) {
  ...
  public NfcReaderDto addReader(NfcReaderDto nfcReaderDto) {
    ...
    NfcReaderDto readerPersisted = this.restTemplate.postForEntity(uri, nfcReaderDto, NfcReaderDto.class).getBody();
  ...
  public List<CornerDto> getOrdersByStatus(Boolean status) {
    ...
    return this.restTemplate.getForEntity(uri, (Class<? extends List<CornerDto>>) ArrayList.class, map).getBody();
  }

  @Override
  public OrderDto callAddMealFromUser(NfcReaderDto nfcReaderDto, NfcTagDto userTag) {
    ...
    return this.restTemplate.postForEntity(uri, null, OrderDto.class, map).getBody();
  }
  ...
  public NfcReaderDto getReader(String tag) {
    ...
    Map<String, String> map = new HashMap<>();
    map.put("tag", tag);
    return this.restTemplate.getForEntity(uri, NfcReaderDto.class, map).getBody();
  }
}
