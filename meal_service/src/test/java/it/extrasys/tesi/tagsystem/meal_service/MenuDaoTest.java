...
public class MenuDaoTest {
  ...
  public void testGetMenuByDate() throws Exception {
    ...
    testMenuEntity.setDate(df.parse("2017-09-12"));
    this.menuDao.save(testMenuEntity);
    this.menuDao.flush();

    MealEntity mealTest = createTestMealEntity();
    this.mealDao.save(mealTest);
    ...
    mealTest = createTestMealEntity();
    // Esecuzione
    this.mealDao.save(mealTest);

    // Verifica
    List<MenuEntity> menuEntities = this.menuDao.findByDate(df.parse("2017-09-12"));
  ...
  public void testGetMenuById() throws Exception {
    ...
    // Esecuzione
    MenuEntity menuEntities = this.menuDao.findOne(testMenuEntity2.getMenuId());

    // Verifica
    assertNotNull(menuEntities);
  }
  ...
  void verifyFinalDatabaseState() {
    ...
    case MENU_EDITED:
      assertThat(this.menuDao.findAll().get(0).getType(), is(equalTo("vegan")));
      break;
    default:
    }
  }
