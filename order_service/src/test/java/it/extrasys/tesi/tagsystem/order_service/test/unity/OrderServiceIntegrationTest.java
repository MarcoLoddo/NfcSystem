...
public class OrderServiceIntegrationTest {
  ...
  public void addConfTest() throws Exception {
    ...
    Mockito.when(this.confManager.addConfiguration(any(ConfigurationEntity.class))).thenReturn(testConf);
    this.mockMvc
        .perform(post("/configurations/").contentType(this.contentType).content(this.jsonConverter.json(createConf())))
  ...
  public void getConfTest() throws Exception {
    ...
    Mockito.when(this.confManager.getConfiguration(Mockito.anyLong())).thenReturn(testConf);

    this.mockMvc.perform(get("/configurations/4")).andExpect(status().isOk())
        .andExpect(content().contentType(this.contentType)).andExpect(jsonPath("$.configurationId", is(1)));
  }
  ...
  public void addOrderTest() throws Exception {
    ...
    Mockito.when(this.orderManager.getById(Mockito.anyLong())).thenReturn(orderEntity);
    Mockito.when(this.restClient.getMeal(Mockito.anyLong())).thenReturn(mealDto);

    Mockito.when(this.orderManager.addOrder(any(OrderEntity.class))).thenReturn(orderEntity);

    this.mockMvc.perform(post("/orders/").contentType(this.contentType).content(this.jsonConverter.json(createOrder())))
        .andExpect(status().isOk()).andExpect(content().contentType(this.contentType))
        .andExpect(jsonPath("$.orderId", is(1))).andExpect(jsonPath("$.totalPrice", is(nullValue())));
  }
  ...
  public void getOrderByIdTest() throws Exception {
    ...
    this.mockMvc.perform(get("/orders/id/1").contentType(this.contentType)).andExpect(status().isOk())
        .andExpect(content().contentType(this.contentType)).andExpect(jsonPath("$.orderId", is(1)));
  }
  ...
  public void getOrderByNfcTest() throws Exception {
    ...
    mockIdOrder(orderEntity);
    Mockito.when(this.orderManager.getByNfc(Mockito.anyString())).thenReturn(orders);

    this.mockMvc.perform(get("/orders/?nfc=" + orderEntity.getNfcId()).contentType(this.contentType))
        .andExpect(status().isOk()).andExpect(content().contentType(this.contentType))
        .andExpect(jsonPath("$", Matchers.hasSize(1))).andExpect(jsonPath("$[0].orderId", is(1)))
  ...
  public void getOrderByDateTest() throws Exception {
    ...
    orders.add(orderEntity);
    mockIdOrder(orderEntity);
    Mockito.when(this.orderManager.getByDate(any())).thenReturn(orders);

    SimpleDateFormat dFormat = new SimpleDateFormat("yyyy-MM-dd");
    this.mockMvc.perform(get("/orders/" + dFormat.format(orderEntity.getData())).contentType(this.contentType))
        .andExpect(status().isOk()).andExpect(content().contentType(this.contentType))
        .andExpect(jsonPath("$", Matchers.hasSize(1))).andExpect(jsonPath("$[0].orderId", is(1)))
        .andExpect(jsonPath("$[0].data", is(dFormat.format(orderEntity.getData()))));
  }
